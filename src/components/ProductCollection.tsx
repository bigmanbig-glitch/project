...
export function ProductCard({ product, delay }: { product: Product; delay: number }) {
  ...
  return (
    <article
      ref={ref}
      className={`reveal ${isVisible ? 'is-visible' : ''} group relative flex h-full min-h-[650px] flex-col overflow-hidden rounded-3xl border border-saffron-500/15 bg-espresso-800/50 transition-all duration-500 hover:-translate-y-1.5 ${accentRing} hover:shadow-gold-lg`}
